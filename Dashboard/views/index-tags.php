...
$lenght = count($tags->all());
$countPage = ceil($lenght / $limit);

$tags_paginate = $tags->paginate($startData, $limit);
$count = $tags->all();
                                ...
                                <table class="w-full whitespace-no-wrap">
                                    <thead>
                                        <tr
                                            ...
                                            <th class="px-4 py-3">Nama Tags</th>
                                    ...
                                    <tbody
                                        ...
                                        <?php foreach ($tags_paginate as $tags): ?>
                                                ...
                                                <?php else: ?>
                                                    <td class="px-4 py-3">
                                                        <div class="flex items-center space-x-4 text-sm">
                                                            <a href="update-tags.php?id=<?= $tags["id_tag"] ?>">
                                                                <button
                                                            ...
                                                            <button onclick="confirmDelete(event, '<?= $tags['id_tag'] ?>')"
                                                                class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                                                aria-label="Delete">
                                                                <svg
                                                                    class="w-5 h-5"
        ...
        function confirmDelete(event, id) {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    setTimeout(() => {
                        window.location.href = `../services/delete-tags.php?id=${id}`;
                    }, 1200)
                } else if (
