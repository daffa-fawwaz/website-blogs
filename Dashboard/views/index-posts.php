...
<body>
    ...
    <div
        ...
        <div id="konten" class="w-full h-full fixed bg-[#0000009d] flex justify-center items-center opacity-0 -z-10 transition-all duration-200 ease-in-out">
            <div class="relative">
                <div class="w-[700px] h-fit bg-gray-600 p-4">
                    <p id="isi-konten" class="text-gray-100"></p>
                </div>
                <div id="close-konten" class="p-2 bg-red-600 rounded-full absolute -top-5 -right-5 cursor-pointer">
    ...
    <script>
        $(document).ready(function() {
            $("#keyword").on("keyup", function() {
                $("#container").load("./../search/Posts.php?cari=" + $("#keyword").val());
            });
        });

        const isiKonten = document.getElementById("isi-konten")
        const konten = document.getElementById("konten")
        const info = document.getElementsByClassName("info")
        const closeKonten = document.getElementById("close-konten")

        function kontenDetail(konten) {
            isiKonten.innerHTML = `${konten}`
        }

        for (let i = 0; i < info.length; i++) {
            info[i].addEventListener("click", function() {
                konten.classList.add("z-50")
                konten.classList.remove("opacity-0")
                konten.classList.remove("-z-10")
            })
        }
        closeKonten.addEventListener("click", function() {
            konten.classList.add("-z-10")
            konten.classList.add("opacity-0")
            konten.classList.remove("z-50")
        })
        ...
        function confirmDelete(event, id) {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    setTimeout(() => {
                        window.location.href = `../services/delete-posts.php?id=${id}`;
                    }, 1200)
                } else if (
